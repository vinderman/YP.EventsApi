...
```bash
docker compose up -d kafka kafka-init kafka-ui users-db events-db bookings-db redis
```
...
Кеширование реализовано в **events-api** через **Redis** (`StackExchange.Redis`) и абстракцию `ICacheService` (`RedisCache`). Кеш снижает нагрузку на PostgreSQL для частых чтений событий и не участвует в защите от овербукинга: резервирование мест по-прежнему идёт через БД с `SELECT FOR UPDATE`.

### Что кешируется

| Ключ | Данные | TTL | Операции |
|------|--------|-----|----------|
| `event:{id}` | Одно событие по идентификатору | 10 минут | Чтение (`GetById`), обновление после мутаций, удаление при `Delete` |
| `events:top10` | Топ продаваемых событий | 10 минут | Только чтение (`GetTopSelledEvents`); актуальность обеспечивается TTL |

Список событий с фильтрацией и пагинацией (`GetAll` / `GET /events`) **не кешируется**: комбинаций фильтров много, а выигрыш от кеша для произвольных выборок невелик.

### Паттерн: cache-aside + обновление при записи

**Чтение** (`GetById`, `GetTopSelledEvents`) — классический **cache-aside (lazy loading)**:

1. Сначала запрос в Redis.
2. При попадании (hit) значение возвращается без обращения к БД.
3. При промахе (miss) данные читаются из PostgreSQL и кладутся в кеш с TTL 10 минут.

**Запись** — не invalidate-only, а **обновление кеша актуальным состоянием** после успешного сохранения в БД:

| Операция | Действие с кешем |
|----------|------------------|
| `Update` | `Set` ключа `event:{id}` обновлённой сущностью |
| `TryReserveSeats` | `Set` после коммита транзакции (актуальный `AvailableSeats`) |
| `ReleaseSeats` | `Set` после коммита транзакции |
| `Delete` | `Delete` ключа `event:{id}` |
| `Create` | Кеш не прогревается — событие попадёт при первом `GetById` |

Так клиенты быстрее получают свежие данные по конкретному событию после изменения мест или атрибутов, без ожидания истечения TTL. Для топа продаж отдельной инвалидации нет: рейтинг меняется постепенно, допустима задержка до 10 минут.

### Почему Redis, а не in-memory

- Кеш **общий для всех экземпляров** events-api (горизонтальное масштабирование).
- Данные переживают перезапуск отдельного процесса приложения.
- Redis уже подключён в `docker-compose.yml` и настраивается через `RedisSettings` (`Server`, опционально `Password`).

### Устойчивость к сбоям Redis

`RedisCache` обрабатывает ошибки и таймауты Redis как деградацию, а не как отказ API:

- неудачный **GET** → `null` (как cache miss) → чтение из БД;
- неудачный **SET** / **DELETE** → предупреждение в лог, запрос к API не падает.

Источник истины остаётся PostgreSQL; при недоступности Redis система продолжает работать медленнее, но корректно.

### Обоснование TTL 10 минут

Десять минут — компромисс между снижением нагрузки на БД и допустимой устареваемостью для каталога событий. Критичные для бронирования изменения мест дополнительно проталкиваются в кеш сразу после транзакции, поэтому TTL в основном страхует от «забытых» ключей и редких расхождений (например, при сбое `Set` после записи).

Поведение кеша покрыто юнит-тестами `EventServiceCacheTests`.
